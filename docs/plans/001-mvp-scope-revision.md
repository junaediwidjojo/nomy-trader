...
- [ ] S13: Implement deterministic candidate filters from approved parameters;
  ...
  - [x] S13m-i: Add a versioned, deterministic reversal-hint filter after
    ...
    cap >$100M, one-day percentage change <=-1%, one-week percentage change
    <=-3%. The one-month percentage change is displayed as context but is not
    a filter. These are a bounded local research
    ...
    research hint under the initial and earlier revised thresholds: BRZE ($26.54;
    1d -12.5%, 1w -19.36%, 1m +0.04%). With the current no-month filter and
    -1%/-3% daily/weekly thresholds, the same snapshot returned BRZE, OI,
    INFY and HPQ. No
...
research thresholds on 2026-09-09, revised to `PCT_CHANGE_1_DAY <= -1%` and
`PCT_CHANGE_1_WEEK <= -3%`; the one-month value is context only, alongside the
