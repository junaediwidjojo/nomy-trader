...
- [ ] S13: Implement deterministic candidate filters from approved parameters;
  ...
  - [x] S13m-i: Add a versioned, deterministic reversal-hint filter after
    ...
    <=-4%. The one-month percentage change is displayed as context but is not
    ...
    1d -12.5%, 1w -19.36%, 1m +0.04%). After the user removed the monthly
    filter, the same snapshot returned BRZE and OI ($6.61; 1d -2.07%,
    1w -6.37%, 1m -6.11%). No
...
research thresholds on 2026-09-09, revised to `PCT_CHANGE_1_DAY <= -2%` and
`PCT_CHANGE_1_WEEK <= -4%`; the one-month value is context only, alongside the
